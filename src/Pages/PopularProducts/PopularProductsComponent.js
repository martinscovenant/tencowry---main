...
const PopularProductsComponent = () => {
  ...
  const categories = [
    'Fashion & Accessories', 'Home Wares', 'Beauty & Personal Care',
    'Electronics & Games', 'Sports', 'General Merchandise'
  ];
  ...
  return (
    <div className='bg-[#f2f2f2] py-8 flex flex-col gap-4'>
      <div className='w-full lg:p-8'>
        <Tabs
          className={`${classes.tabs} overflow-x-scroll`}
          indicatorColor={""}
        ...
        >
          {categories.map((category, index) => (
            <Tab
              key={index}
              color='secondary'
              className='tab-button sm:text-xs'
              label={category}
              {...a11yProps(index)}
            />
          ))}
